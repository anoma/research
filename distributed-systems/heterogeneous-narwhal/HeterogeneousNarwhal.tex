...
anchor block selection in the Narwhal \Dag. %
...
\item %
  ...
  it with all its causal predecessors that were not committed yet. %
\end{enumerate}
However, %
as learners are heterogeneous, %
they might (want to) progress at different ``speeds''. %
In particular, %
...
\item[progression] %
  the chosen anchor block must \emph{not} be reachable via references %
...
\item[readiness] there is at least one learner for which %
  \begin{itemize}
  \item the underlying header contains a signed quorum and %
  \item the previous anchor block has been committed %
    / ordered (unless it is the first anchor block). % 
  \end{itemize}
\end{description}
