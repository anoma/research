...
\item[\code{WorkerHx}Worker hash reception (\wh←)]
  ...
  When a worker hash is received (that was compiled by a local worker) is received,
...
\item the round number 
\item a non-empty\todo{discuss with isaac} list of signed quorums\todo{subject
...
\item[Block reception (\bk←)]
  ...
  they all carry the same round number, 
  ...
  \item[Signed quorum broadcast (\bk!\qs⇒)] 
    ...
    (the order being the local order of reception). 
  ...
  \item[Header announcement ((\bk/\wh/\ac)!\(\hd^*\)⇒)]
    ...
    a new block header fingerprint is announced. 
...
question in my head :
can we separate out reliable broadcast , which seems to be happening in the availability protocol ? 
