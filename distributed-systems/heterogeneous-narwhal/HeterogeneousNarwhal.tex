...
\item[%
  ...
  Transaction request collection (\onetx←)%
  ]
  \tnote{worker\\ ← client}
  ...
  to control the rate of clients requests. %
  \endnote{%
  ...
  \item[%
    \code{???}%
    {Transaction batching (\(\tx:= \tx:\onetx\))}%
    ]%
    \tnote{[worker]}%
    The worker adds the transaction request \onetx to the current batch \tx. %
    This might trigger the completion of the batch. %
  \item[%
    \code{???}%
    {Batch completion (\tx{}!)}%
    ]%    
    \tnote{[worker]!}%
    Completion of the batch results in
    a number of new messages to be sent, 
    as detailed in the section on batch distribution.
    Moreover, %
    the counter for batches is increased. 
  \end{description}
\item[%
  Batch distribution (\tx⇒)%
  ]
  \tnote{worker\\ ⇒ worker}
  ...
  this is the approach that we focus on in this paper,
  \ie all erasure coding shares are full copies.
  In general,
