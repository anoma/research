...
\usepackage[textsize=footnotesize]{./myluatodonotes}%\usepackage{todonotes}%
...
\cite{anoma_specs}\todo{%
  check / update link, % 
  \url{https://specs.anoma.net/main/components/typhon/execution.html?highlight=exe\#execution}
}, %
...
The \hnr protocol uses learner-specific assumptions,
...
We start by fixing a finite set of \emph{learners}. %
each of which has their own assumptions about the behavior of validators.\todo{%
  double check with Isaac %
  that it is safe to use the terms %
  validator and acceptor %
  interchangeably 
} %
Each learner is asked to commit to a set of \emph{quorums}\todo{define\&explain} 
such that, % 
at any given point in time, %
at least one of the quorums will be live for a sufficiently long period. %
\endnote{%
...
\begin{definition}[Learner-specific quorums]
  \label{def:trustlive}
  Each learner \(a\) chooses a set of quorums \(\Q{a}\)
  such that one of these quorums is live at all times %
  (according to their beliefs).\xspace%
   \footnote{%
...
\label{sec:overview}
\todo[inline,caption={}]{probably move the ``hidden material'' to an earlier point (context)}
\begin{comment}
