...
as low as possible.
In particular,
primaries never send messages or any other signals to their workers.\xspace%
\footnote{%
  However, %
  there will be a point in time when workers receive signals
  after transactions have been processes
  such that they can free up the storage that they use for 
  keeping transaction data available. 
}%
\endnote{%
...
\todo[inline]{possibly move to }
In the high-level protocol description, %
one typically considers the benevolent scenarios %
where all message dependencies are met, %
such that all messages that one needs to progress, %
i.e., compile and send new messages, %
have already been received and their data are immediately accessible. %
However, eventually one has to also address the issues of %
managing message dependencies. %


\begin{description}
\item[filtering] how we can efficiently sort out unwanted messages
\item[pre-condition check] whether the message triggers one or more actions 
\item[processing] what we do if the pre-condition is met
\item[post-condition assertions] if applicable, what is ensured by the action. 
\end{description}
...
Already at genesis,
the general situation becomes \st{slightly more complex}  \todo[inline]{\Large a nightmare}. 
