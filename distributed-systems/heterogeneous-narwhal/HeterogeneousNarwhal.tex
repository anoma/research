...
their role in the integrity protocol concerns merely “bookkeeping” matters. % 
Specifically,
...
\todo[inline,color=yellow!80!white]{
  ...
  We might also want to defer this to
  Section/Appendix\ref{sec:mess-depend-manag}
}
\begin{description}
\item[%
  ...
  \item[%
    ...
    and the sequence of batches are numbered sequentially. 
    Within a batch, %
    transactions are assigned sequence numbers; %
    the sequence number of the transaction is essentially
    the position in the current batch. %
    Thus,
    each transaction request is tagged with a unique pair of a
    \emph{batch number} and a \emph{sequence number}. 
  \item[%
    \code{TxToAll}%
    ...
    the message also contains the current batch and %
    the sequence number of the transaction within the batch. %
  \item[%
...
\item[%
  ...
  \item[Transaction storage] 
    ...
    \begin{itemize}
    \item the id of the collecting worker, 
    \item the batch number of the batch to which it belongs, and
    \item the sequence number of the batch to which it belongs.
    \end{itemize}
  ...
  \item[\code{WHxFwd}{Worker hash (\es!\wh[]↑)}] 
    \xnote{%
      worker\\
      →primary%
    }
...
\item[%
  ...
  \item[%
    ...
    that it has already stored the transactions of a received worker hash, %
...
messages, %
giving also some details of how message dependencies lead to slightly different cases.
...
We first spell out both protocols at genesis
...
\item[\code{WorkerHx}Worker hash arrival (\wh←)]
  ...
  \item[\code{NextHeader}{Header announcement (\wh!\hd[]⇒)}]%
    ...
    \item a list of pairs of a batch number and a worker id.
    \end{itemize}
...
\item[{\code{WorkerHx}Worker hash forward (\wh[]←)}]
  ...
  \item[{Header signature commitment (\wh[]!\(\hd_{\sim}\)→)}] %
    ...
    If the received worker hash completes was the last worker hash
    that matches a previously received header fingerprint, %
    the primary should commit to the header by signing it
    and sending it back to the creator of the header fingerprint. %
...
\item[{Header announcement / signature request (\(\hd[]^*\)←)}] %
  ...
  \item[{Header signature commitment (\(\hd[]^*!\hd_{\sim}\)→)}] %
    ...
    If all referenced worker hashes have already been forwarded, %
    the primary commits to the header by answering with a signature over the
    header. 
  \end{description}

  \todo[inline]{%
    we need flow diagrams to get rid of the ``code duplication'' !!!%
  }
  % CONTINUE HERE 
