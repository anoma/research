...
\section{{How to superimpose two Narwhal DAGs?}}
\label{sec:context}%
\label{sec:running-example}%
%
Recall that the Narwhal mempool protocol, conceptually, %
...
\foreach \colour/\flip/\sh in {black/1/0} {
  \begin{scope}[opacity=1,shift={(\sh,0)},xscale=\flip]
    ...
    %             % \uncover<\showon->
                 {%
                  ...
                  \draw[thick,color=\colour,double,opacity=1]
                  (b\v-\rrr_\l.center) 
...
The validators set out to build a single shared Narwhal \Dag that comprises %
the information of \emph{two} separate ``native'' Narwhal \Dag[s]. %
The rationale is that then blocks may contain cross-chain transactions %
that can be executed atomically. %

A first idea of %
what such a ``super-imposed'' \Dag looks like is %
illustrated in Figure~\ref{fig:superposition-dag}); % 
the colored circles under the validators, %
rendered as server icons ({\tiny\faServer}), %
represent the stake they hold, either blue, red or both. %
Now, %
as a first thought, %
we can restrict anchor blocks to those that %
satisfy the conditions for both \base[s]; %
in fact, %
we obtain a safe implementation of a combined Narwhal mempool. %
However, %
apart from obvious questions about skewed incentives for %
the validators that only hold stake of one of the \base[s], %
the ``perfect'' safety guarantees of the naïve superposition %
come at the price of risks for liveness: %
roughly, superimposing Narwhal \Dag[s] leads to a %
``less live'' \Dag, relative to \emph{each one} of the \base[s]. %
We shall describe how we can do better,
not only for this specific example,
but more generally for any system based on a valid learner graph.\footnote{%
  In fact, %
  there is a whole spectrum of trade-offs for building a shared mem-\Dag, %
...
\begin{figure}[htb]
  ...
  \caption{Naïve Superposition of two Narwhal \Dag[s]}
...
\todo[inline,size=normalsize,caption={},color=yellow]{%
  \begin{minipage}{1.0\linewidth}
    \begin{itemize}
    \item %
      the context section needs to be written after %
      we've narrowed down the audience %
    \item %
...
\section{Learner-graphs, quorums, and all that} 
...
\todo{check again if this makes sense to the average person}
...
are hash references to batches of transaction requests, 
...
\item[%
  ...
  \item[%
    ...
    which is a list of transaction requests (initially empty). %
    % The worker adds the transaction request \tx to the current batch \txs. %
    Batching happens unconditionally, % 
    ...
    Thus,
    each transaction request can be identified by %
    its \emph{batch number}, %
    \emph{sequence number} and %
    the worker \textsc{id} %
    of the worker that received the transaction request; %
    we call this triple the \emph{fingerprint} of the transaction request.\xspace%
    \footnote{%
      The order of transactions within a batch %
      might be permuted before execution to avoid \mev potential. %
    }
  \item[%
    \code{TxToAll}%
    ...
    unconditionally; % 
    the copy of the received transaction request is broadcast together with its fingerprint.
  ...
  \item[%
    ...
    In principle, %
    the decision for %
    when to produce and send a new worker hash is made by the worker %
    (as long as each worker hash contains at least one transaction %
    and does not exceed a potential maximum size).\xspace%
    At which exact moment worker hashes are compiled can depend on %
    several factors, %
    \eg on a maximum number of transaction requests per worker hash  %
    or a maximum delay between %
    the first and the last transaction request within a worker hash. %

  \item[\code{MessageEnum::TxAck}
    Transaction request acknowledgment]
    Optionally, %
    one may acknowledge the client's requests, %
    say by a signature of the transaction data and a worker local time stamp. %
    \endnote{%
      In the
...
\item[%
  ...
  \begin{itemize}
    ...
    Note that copies of transactions are unsigned. %
    Signatures are deferred to the corresponding worker hash. %
    Moreover,
    storing transaction copies to be retrievable via their fingerprint %
    will be useful for handling “foreign” worker hashes, %
    \ie worker hashes that refer to transactions that are collected by mirror workers.%
    \begin{description}
    ...
    If the received transaction copy was %
    the last missing one to match a previously received worker hash, %
    the receiving worker sends the worker hash to its primary. %
    The \mbox{(pre-)}\allowbreak condition\xspace%
    \todo{%
    ...
    \footnote{%
      This might be an unexpected order of events, %
...
\item[%
  ...
  it first checks the signature and then responds %
  ...
  (and the worker hash forwarding will be triggered by the last transaction copy).%
  \endnote{%
...
Thus, %
sending a message might be due to several, slightly different types of scenarios %
(depending on which message from %
a \emph{set} of triggering messages arrives last and %
thus becomes {the} trigger). %
Note that there is a duplication in actions to be taken in response to %
a received message, %
which can be avoided by collecting all potential triggers of a message; %
we shall combine %
\((\es)\)!\wh↑ and \((\wh[])\)!\wh↑) into \((\wh[] / \es)\)!\wh↑. %
Thus, the forwarding of worker hashes is then described succinctly as follows. 
\begin{description}
\item[%
  \code{MessageEnum::WHxFwd}%
  Worker hash forwarding {(\((\wh[] / \es)\))}%
  ]
  \xnote{worker\\ → primary} 
  If a worker has already stored the transactions of a received worker hash, %
  it sends the worker hash to its primary.\footnote{%
...
\endnote{
  we might want a section on {Execution support} eventually
}
...
We have seen this already in the worker protocol, %
where the primary trigger for worker hash forwarding are worker hashes, %
...
\item[\code{WorkerHx}Worker hash completion (\wh←)]
  ...
  \item[\code{NextHeader}{Header announcement (\wh!\hd[]⇒)}]%
    ...
    The primary may\footnote{%
      This depends on when the primary considers it is “time” to do so. %
    }
    announce the next header %
    ...
    The message of the header announcement only contains
...
\item[{\code{WorkerHx}Worker hash reception (\wh[]←)}]
  ...
  the primary adds the worker hash to the current set of known worker hashes %
  (initially empty).
  % Primaries keep a set of \emph{active} worker hashes for each validator %
  % and also a set of worker hash fingerprints. %

  \begin{description}
  \item[{Header commitment (\wh[]!\(\hd_{\sim}\)→)}] %
    \xnote{primary\\
      →primary
    }
    If the received worker hash completes the set of known worker hashes
...
\item[{Header reception (\(\hd[]^*\)←)}] %
  ...
  \begin{description}
  \item[{Header commitment (\(\hd[]^*!\hd_{\sim}\)→)}] %
    \xnote{%
      primary\\
      →primary
    }
...
\item[Header signature (\hdₚ←)]
  ...
  \item[{Availability certificate broadcast %
    ...
    If the received signature completes a global weak quorum, %
...
which include  references to a quorum of blocks. %
We have \emph{learner-specific blocks}, %
which contain \emph{signed block quorums}, %
defined as follows:  
...
\item the round number 
\item a list of signed quorums,\endnote{%
    how can we ensure in theory that we make progress ?
  }
  at most one per learner. 
\end{itemize}
The first two items were already present in genesis block headers
while the remaining three come into play in later rounds. 
...
\item[Block reception (\bk←)]
  ...
  If a received learner-specific blocks completes a quorum of blocks
  ...
  \item[Signed quorum broadcast (\bk!\qs⇒)] 
    ...
    (\eg in the order that they were received)
  ...
  \item[Header announcement (\((\bk/\wh/\ac)\)!\(\hd^*\)⇒)]
    ...
    \item the availability certificate being the missing data,
      ...
      signed quorums (of maximal learner-specific heights),
      and as many worker hashes as allowed to be included in the header. %
    \end{itemize}
  \end{description}
...
becomes a chain of commitments
