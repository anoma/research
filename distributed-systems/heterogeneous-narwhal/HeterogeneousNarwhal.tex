...
The validators agree to build a single shared Narwhal \Dag that comprises %
the information of \emph{two} separate Narwhal \Dag[s] %
such that blocks contain cross-chain transactions that can be executed atomically. %
...
the additional safety guarantees come at the price of %
risks for liveness: %
roughly, this naïve superposition of Narwhal \Dag[s] tends to be %
``less live'' than each of the \base[s]. %
We shall describe how one can do better for this specific example %
and, in general, %
the whole spectrum of trade-offs for building a shared mem-\Dag %
for a pair of \base[s]. % 
