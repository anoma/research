...
So, do we want to add just yet another \Dag-based consensus protocol?
...
consensus protocol.
The present paper rather explains how to heterogenize existing protocols,
starting with one paradigmatic example.\footnote{
  We are confident that the heterogenization process applies to the whole family of \Dag-based mempools.
...
is the collection of all sets of validators that are staking more than
⅔ of all stake of the respective \base. 
...
\begin{enumerate}
\item there is a common set of acceptors \(\Accs\),
...
we also would like learners to commit to assumptions about (absence of) Byzantine behavior %
such that we can state conditions under which %
\Dag properties of the Narwhal protocol will transfer to \hnr. %
Roughly, %
we are looking for  validator sets %
that are “big enough” and consists of %
“sufficiently trustworthy” validators % (according to a given learner) %
such that learners would expect consistency between pairs of \base[s]. %

If we want to be more precise,
we of course have to take into account that consistency is relative to the choice of pairs of \base[s]. %
Moreover learners might have different opinions of whether a %
specific validator (set) is trustworthy enough or %
even has an intrinsic interest in consistency between a pair of \base[s]. %
However, %
before we go through all the details of whose opinions about what in relation to
pairs of \base[s] we are interested in, %
let us point out the simple default assumption: %
a \emph{default safe set} that can ensure consistency of two \base[s] \(a,b∈\Lrnrs\) %
is a set of validators \(s\subseteq \Accs\) that satisfies \(s \cap \q{a} \cap \q{b} \neq
\varnothing\)
for every pair of learner specific quorums \(\q{a} \in \Q{a}\) and \(\q{b} \in
\Q{b}\). 
The definition of a learner graph is more general,
but specializes to the case of default safe sets forming the edges between learners. %
...
\begin{definition}[Learner graphs, validity of learner graphs]
  ...
  validators \(\Accs\) is a pair \((\Q{}, \_–\_)\) where
  \(\Q{}\) is a quorum system and %
  \({\_–\_} \colon \Lrnrs \times \Lrnrs \to \wp\Bigl(\wp(\Accs)\Bigr)\)
  is a function such that the following hold, for all \(a,b \in \Lrnrs\),
  for all \(s \in a–b\) and \(x \in \Accs\):
  \begin{enumerate}
  \item \(a–b = b–a\) %
  \item \(s \cup \{x\} \in  a–b\). %
  \end{enumerate}
  ...
  % \end{verbatim}
  A \emph{safe set} of a learner graph is an element of \(a–b\), for some \(a,b \in \Lrnrs\). %
\end{definition}
...
nodes are moreover labeled by the set of all learner-specific quorums of the
respective learner. 
Further discussion of the learner graph is deferred to
Appendix~\ref{apx:learner-graph}. 
