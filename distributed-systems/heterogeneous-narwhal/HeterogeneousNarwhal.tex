...
We start by fixing a finite set of \emph{learners} \(\Lrnrs\); %
...
\item there is a common set of acceptors \(\Accs\),
  obtained as the union of the (unions of the) learner-specific quorums and that  
\item 
...
the reasoning is that if the larger set \(qₐ \cup \{x\}\) is live, %
then in particular the ``original'' quorum \(qₐ\) must be live. %
\endnote{%
...
we also require assumptions about safety, %
which we need later to describe how and under which conditions %
the \Dag properties of the Narwhal protocol %
transfer to \hnr. 
...
validator:\(\left\{%
  \begin{tikzpicture}[baseline={(q.north)},remember picture,yscale=-1]
