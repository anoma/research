...
\item[%
  ...
  \endnote{%
    ...
    in particular details from the \ptop layer.
  }
  ...
  \item[%
    ...
    \ie there is always a single current transaction batch,
    which might be possibly empty. 
    Adding the received transaction request might trigger the completion of the batch. %
    The current batch is locally identified by an integer, %
    called the current \emph{take}. %
  \item[%
    \code{???}%
    {Batch completion (\tx{}!)}%
    ]%    
    \tnote{[worker]!}%
    Completion of the batch triggers
    \begin{enumerate}
    \item broadcast of a new worker hash (as detailed next)
...
\item[%
  ...
  \tnote{worker\\ ⇒ worker}
  Upon completion of a batch of transaction requests,\xspace%
  \footnote{%
    Note that batch completion is triggered by a client request. %
  }
  a worker ``broadcasts''\xspace%
  \footnote{%
