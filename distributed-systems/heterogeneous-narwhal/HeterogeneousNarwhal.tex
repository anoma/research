...
\begin{ex}[Broadcasting certificates of availability (\textsc{i/ii})]
  ...
  \begin{enumerate}
  \item count down as “shared counter”, starting from \(f+1\);
  \item synthesize the certificate of availability (and broadcast it);
  \item count down further and free the counter if it reaches~\(-2f\).
  \end{enumerate}
\end{ex}
...
\begin{ex}[Broadcasting certificates of availability (\textsc{ii/ii})]
  ...
  We could simply use one counter for each quorum; %
  however, that would lead to an exponential blow-up. %
  Hence,
  we make use of the fact that
  each message falls into exactly one “region”\footnote{
    There is a one to one correspondence of regions and terms
    in the inclusion-exclusion formula,
    and vice versa.
  }\todo{%
    1. explain the regions of the Venn diagram properly\\
    2. explain the relation to the inclusion-exclusion principle
  }%
...
\endnote{%
  ...
  %
  We can use the
  \href{https://en.wikipedia.org/wiki/Inclusion\%E2\%80\%93exclusion_principle}{inclusion
    exclusion principle.}%
  We can organize the regions of the Venn diagram into a tree %
  in which
  \begin{itemize}
  \item the root is the region in which all sets intersect
  \item the leaves are the regions sets of elements that belong to exactly one
    set
  \item there is a path from one region to another if the former intersects with
    more sets than the latter
  \item in particular, direct ancestry means to have exactly one additional intersection
  \end{itemize}
  ...
  yields a single node if we start with the set of all sub-sets?
}
  \begin{enumerate}
  \item count down as “shared counter”, starting from \(f+1\);
  \item synthesize the certificate of availability (and broadcast it);
