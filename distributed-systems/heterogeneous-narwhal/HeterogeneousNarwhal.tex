...
\newunicodechar{ₚ}{\ifmmode_a\else\ensuremath{{}_q}\fi}
\newunicodechar{‼}{\rough}
\newunicodechar{‽}{\ensuremath{?!}}
...
\footnote{% 
  We keep the terminology introduced for %
  Heterogeneous Paxos \cite{opodis20HPaxos}. %
} %
We start by fixing a finite set of \emph{learners} \(Learners\), %
each of which holds certain beliefs that 
imply restrictions on the behavior of validators that is considered possible.\xspace%
\todo{%
...
from learners to sets of quorums. 
...
\begin{definition}[Learner-specific quorum system]
  ...
  and a learner \(a\), 
  we denote the learners set of qourums by \(\Q{a}\), 
  whose elements are the \emph{learner-specific quorum} of \(a\).
  We use \(qₐ, q'ₐ,\) \etc to range over learner-specific quorums of learner \(a\).
\end{definition} %
One might even go as far and ask learners to choose %
a maximal set of quorums that are compatible with their beliefs; %
however, %
all we need is a map from learners to sets of quorums. %
Based on these we can formulate the remaining two core definitions. %
...
and feature in the  integrity protocol only insofar as 
they take care of “bookkeeping”. 
Specifically,
worker keep track of (batches of) transactions,
their hashes,
...
\todo[inline]{%
  ...
  differently to what what is/was in the specs
...
\item[%
  ...
  The worker possibly has to react to the transaction request, 
  ...
  \item[%
    ...
    every worker stores \emph{the} current batch \txs, %
    ...
    The current batch is locally identified by an integer, %
    which we dub \emph{take}; %
    within the take, transactions have consecutive sequence numbers, %
    \eg the position in the transaction buffer (seen as a list). 
  \item[%
    \code{TxToAll}%
    ...
    In principle,
    we could use arbitrary erasure coding shares.
    However,
    ...
    we visually distinguish “copies” of transaction batches
    ...
    the message also contains the current take and the sequence number within the take. 
  \item[%
    \code{WHxToAll,WorkerHash}%
    {Worker hash broadcast trigger (\tx{!}\wh{}⇒)}%
    ]%    
    \tnote{worker\\ ⇒ worker}
    The received  transaction request might trigger a new worker hash to be produced %
...
%   erasure coding shares of %
%   the underlying transaction data to mirror workers.
%   In the simplest case, 
%   this amounts to broadcasting full copies of the transaction data;
%   this is the approach that we shall focus on in this paper,
%   \ie all erasure coding shares will be full copies
%   such that a single honest worker will suffice to retriever the data. 
%   In general,
%   transaction distribution can be divided into two steps.
%   \todo[inline]{
%     we might require a
%     \emph{position number} for transactions of the current batch, 
%     and a sequence number (of the block to be produced)
%   }
%   \begin{description}
%   \item[Erasure coding via copying (\es)]
%     \tnote{[worker]}
%     Recall that erasure coding will be trivial %
%     as described for the homogeneous case~\cite{NT}. %
...
\begin{figure}[htb]
  \centering
  \todo[inline]{need to ``fix'' the \tx subscripts to be sequence numbers;
    possibly adding also the take number of the worker hash?
  }
  %
