...
\todo[inline,caption={},color=green!30!white]{%
  ...
  \item[learner graph] %
    ...
    \item
      ...
      which hands out such a learner-craftable graph to the consensus designer,
      thus avoiding the assumption that a learner is in general asked to craft
      its learner graph (such a \emph{user} will possibly make .. mistakes)
    \item
      \emph{observed} learner graphs,
      typically generated by validators, 
      which keep track of what they have seen;
...
\begin{abstract}
  ...
  which  roughly means eventual execution of its transactions. 
...
which
\begin{itemize}
\item everyone can interact with all chains of the ecosystems and 
\item there is a unique definitive state of every ``base layer''.
\end{itemize}

For technical considerations,
the main points (for validators and/or executors) are
\begin{itemize}
\item I can participate in the production of as many ``base layers'' as I want
\item as long as they keep a record of relevant transaction data. 
\end{itemize}
